...
class INSwapper():
    ...
    def get(self, imgs, target_faces, source_faces, paste_back=True):
        assert len(imgs) == len(target_faces) == len(source_faces), "The number of images, target faces, and source faces must be the same."
        ...
        preds = self.session.run(self.output_names, {self.input_names[0]: blobs, self.input_names[1]: latents})[0]
        #print(latent.shape, latent.dtype, pred.shape)
        for i in range(len(imgs)):
            img_fake = preds[i].transpose((1,2,0))
            bgr_fake = np.clip(255 * img_fake, 0, 255).astype(np.uint8)[:,:,::-1]
            bgr_fakes.append(bgr_fake)
            if not paste_back:
                continue
                #return bgr_fake, M
            else:
                ...
                fake_merged = img_mask * bgr_fake + (1-img_mask) * imgs[i].astype(np.float32)
                fake_merged = fake_merged.astype(np.uint8)
                fake_mergeds.append(fake_merged)
                #return fake_merged
        if paste_back:
            return bgr_fakes, fake_mergeds
        else:
            return bgr_fakes
